...
- `data/profiles/*.yaml`: Data-driven methodology profiles (`ejptv2`, `network_pentest`, `web_pentest`, `htb_lab`). Each profile declares ordered phases with `checklist_categories` routing, `depends_on` sequencing, `prerequisites` (non-linear branch jumps via `target_status` / `evidence_type` conditions), and `evidence_required` completion gates. New methodologies are added by dropping a YAML file in this directory — no core code changes.
- `profile.py`: Pydantic models (`MethodologyProfile`, `PhaseDefinition`) and `ProfileLoader` (bundled + custom YAML loading).
...
- `get_next_actions(...)`: prioritized investigations with rationale. Ordering: phase-0 gaps → confirmed-access opportunities (valid admin creds, confirmed findings, owned-host `privesc` nudges) → untested enumeration surface → credential sprays → interrupted work → stale-lead housekeeping. Deterministically deduplicated and sorted; out-of-scope and ignored targets are excluded. A FOOTHOLD host with zero pending/running checks always yields a `privesc` nudge so the post-exploit phase never goes silent (findings don't suppress it — they are already retired for owned hosts). Consumed by the TUI triage board, the live `NEXT:` stats-banner hint, and the CLI `synapse next` command.
- `detect_rabbit_holes(...)`: `StuckReport` separating proven dead ends from untouched surface and un-sprayed credentials; `is_stuck` is the rabbit-hole signature (dead ends exist AND no open avenue remains in scope), suppressed when every in-scope host is PWNED — full ownership is completion, not a trap. Powers the TUI "I'm stuck" modal (key `s`); triage board on key `n`; live `NEXT:` hint in the stats banner consumes `get_top_action`.
- `evaluate_phase_progress(target, profile, evidence)`: pure guided-workflow evaluation against a `MethodologyProfile`. Routes checklist items to phases by category, marks phases BLOCKED until dependencies settle (empty stages pass through only when their upstream spine is equally clear), unlocks branches early via prerequisites (e.g. foothold status jumps straight to privilege escalation), and holds evidence-gated phases (user/root flags) open until proof is captured — but only once routed surface exists: on a pristine pre-recon target the gates stay dormant instead of nagging for proof flags before recon has run. Emits per-phase `PhaseProgress` (completed/pending/running checks, dead ends, findings, evidence, rationale-backed `NextAction`s). Consumed by the Guided Workflow modal (TUI key `g`); contains no eJPT-specific behavior — all phase semantics live in profile YAML.

The synthetic pentest lab under `lab/` (Docker Compose) exercises this engine end-to-end; `tests/test_lab_scenario.py` asserts the documented per-stage state machine behavior against it.
...
- `app.py`: Main Textual application with responsive layout, tabs (`1`–`5`), stats banner (with live `NEXT:` hint), state-aware triage (`n`), stuck/rabbit-hole modal (`s`), scope toggling (`o`), credential lifecycle marking (`t` on the Creds tab), methodology profile switching (`p`, persisted via the `active_profile_id` metadata key with stale-profile fallback), and the guided workflow breakdown (`g`, live per-phase progress for the selected target). Service status is derived automatically from checklist state (`_refresh_service_state`).
...
- `modals/`: Isolated modal dialog screens with input focus safety, including `TriageModal`, `StuckModal`, `ProfileModal` (loader-backed profile switcher), and `GuidedPhaseModal` (per-phase completed/pending/running checks, dead ends, findings, evidence, and rationale-backed recommendations).
